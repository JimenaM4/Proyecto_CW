<?php 
    ...
    if(!$conexion)
    {
        ...
        $sql =  "SELECT * FROM ventas WHERE ID_venta = $_GET[id]";
        $res = mysqli_query($conexion, $sql);
        $respuesta = [];
        while( $datos = mysqli_fetch_array($res)){
            $respuesta[] = $datos;
        }
        // echo $respuesta[0][1]."<br>";//ID del usuario que creó la publicación
        // echo $respuesta[0][2]."<br>";//descripcion
        // echo $respuesta[0][3]."<br>";//precio
        // echo $respuesta[0][4]."<br>";//lugar
        // echo $respuesta[0][5]."<br>";//imagen
        // echo $respuesta[0][6]."<br>";//usuario que creó la publicación
        // echo $respuesta[0][7]."<br>";//hora1
        // echo $respuesta[0][8]."<br>";//hora2
        // echo $respuesta[0][9]."<br>";//telefono
        // echo $respuesta[0][10]."<br>";//nombre del producto
        echo "<!DOCTYPE html>
            <html lang='en'>
            <head>
                <meta charset='UTF-8'>
        ...
        echo '<body>
            ...
            <main id='publicacion' class='rounded mx-auto d-block'>
                    <div id='publi' align='center'>
                        <h1>{$respuesta[0][10]}</h1>
                        <h2>{$respuesta[0][2]}</h2>
                        <h2>Precio: $ {$respuesta[0][3]}</h2>
                        <h2>Lugar de entrega: {$respuesta[0][4]}</h2>
                        <h2>Contacto: {$respuesta[0][9]}</h2>
                        <h2>Horario de entrega: {$respuesta[0][7]} - {$respuesta[0][8]}</h2>
                        <h2>Publicación creada por: {$respuesta[0][6]}</h2>
                        <img id='img' src='{$respuesta[0][5]}' alt='Producto' style='max-width: 300px; height: auto;'><br><br>
